...
public class GUHUtils{
	...
	public static String resolveBarCode(String barCode){
		...
		_log.debug("Found a match for the bar code " + barCode+ " --> " + match);
		return match; 
	}
	...
	private static List<String> segmentBarCode(String barCode){
		...
		if(barCode.startsWith("[)>\u001e06")){
			_log.debug("barCode contains header ");
			barCode = barCode.substring(6, barCode.length());
			if(barCode.contains("\u001D")){
				_log.debug("barCode contains \u001D  ");
			...
			_log.debug("barCode doesn't contain header but barCode contains | ");
			barCodeSegments = Arrays.asList(barCode.split("\\|")); 
		}
		else{
			_log.debug("barCode doesn't contain header");
			barCodeSegments.add(barCode);
		}
	...
	private static String checkBarCodesInHBaseDatabase(List<String> barCodesToCheck){
		...
		_log.debug("query to be fired  checkBarCodesInHBaseDatabase --> " + query);
		...
		ResultSet delphiRS = null;
		 try{
			delphiConn = AptivHbaseServerConnection.getConnection();
			...
			AptivHbaseServerConnection.close(delphiConn, delphiStmt, delphiRS);
		}		
